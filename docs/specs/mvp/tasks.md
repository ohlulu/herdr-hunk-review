...
- [x] T001 [REQ-001, REQ-006, REQ-009] Create `herdr-plugin.toml` per DEC-003: id `herdr-review`, `min_herdr_version = "0.8.0"`, one `[[panes]]` entry `picker` (placement split, `sh -c` exec via `$HERDR_PLUGIN_ROOT`), two `[[actions]]` `review` / `send-notes` running `python3 scripts/review.py open-picker|send-notes`. Verify: `herdr plugin link ~/Developer/ohlulu/herdr-review && herdr plugin list | grep -q herdr-review` → exit 0
- [x] T002 [REQ-001..008] Create `scripts/review.py` skeleton (DEC-001, DEC-002): stdlib-only, subcommand dispatch (`open-picker` / `picker` / `send-notes`), thin subprocess runners (`run_herdr` / `run_git` / `run_hunk`) kept separate from pure decision functions so tests can inject fakes, atomic JSON state read/write (temp + rename) under `HERDR_PLUGIN_STATE_DIR`. Verify: `python3 -m py_compile scripts/review.py && ! python3 scripts/review.py bogus 2>/dev/null` → exit 0
- [x] T003 [REQ-004, REQ-008] Create `tests/test_review.py` (stdlib unittest, `sys.path` shim to `scripts/`) with state IO tests: missing file → default, write-then-read roundtrip, temp+rename atomic replace. Verify: `python3 -m unittest discover -s tests` → OK

## Phase 2: Picker entry (REQ-001)

- [x] T004 [REQ-001] Implement `open-picker` action (DEC-004): pure `resolve_review_cwd(context_json, pane_list)` reading `HERDR_PLUGIN_CONTEXT_JSON.focused_pane_cwd` with `herdr pane list` focused-pane fallback; then `herdr plugin pane open --plugin herdr-review --entrypoint picker --placement split --direction right --cwd <cwd> --focus`; failures → `herdr notification show` + stderr (DEC-011). Tests cover context-present and fallback paths. Verify: `python3 -m unittest discover -s tests` → GREEN
- [x] T005 [REQ-001] Implement picker startup guard: normalize cwd via `git rev-parse --show-toplevel`; non-repo → print `not a git repository`, wait for one keypress — tty: raw single-byte read via `termios`/`tty` cbreak with restore in `finally`; non-tty: `readline()` — then exit 0 (AC-002, DEC-011). Verify: `cd /tmp && printf '\n' | python3 ~/Developer/ohlulu/herdr-review/scripts/review.py picker | grep -q 'not a git repository'` → exit 0

## Phase 3: Menu + base resolution (REQ-002, REQ-003)

- [x] T006 [REQ-003] Implement `resolve_base(git)` pure function over an injected git runner: non-own-tracking `@{u}` first, then fork-parent detection (DEC-013: first-parent fork point + best containing ref; children/twins containing HEAD, own remote copies, and remote HEAD aliases excluded; skipped on trunk branches), then `origin/HEAD`, then first existing of `main` / `master` / `trunk`, else None. Tests cover AC-006 (upstream-self skip), AC-017–AC-019 (unit + real-git integration), and all-fail → None. Verify: `python3 -m unittest discover -s tests` → GREEN *(amended 2026-08-18: originally shipped without fork-parent detection)*
...
- [x] T008 [REQ-002] Wire interactive fzf flows (DEC-006): main menu fzf (first row default); Pick commit `git log --oneline --color=always -200 | fzf --ansi`; Pick range two rounds over the same log (`old>` picks the range base, `new>` lists only commits newer than it plus a leading `(worktree)` default row → `old..new`, worktree row → that commit vs worktree); Branch vs branch two-round fzf (`base>` then `compare>`) over `git branch -a --format='%(refname:short)' --sort=-committerdate` (`-a` required for remote-tracking branches; filter out the `origin/HEAD` alias row); Esc at any stage → exit 0. Verify: `python3 -m py_compile scripts/review.py` → exit 0 (interactive UX verified in Human Acceptance) *(amended 2026-08-18: originally shipped as single-round `--multi 2`)*
...
- [x] T016 [REQ-009] Add herdr `[[keys.command]]` entries in `~/.config/herdr/config.toml`: `["prefix+alt+h", "ctrl+alt+shift+h"]` → `plugin_action herdr-review.review`, `["prefix+alt+s", "ctrl+alt+shift+s"]` → `plugin_action herdr-review.send-notes`, comment style per existing lazygit/yazi entries, then reload. Verify: `herdr server reload-config && grep -c 'herdr-review\.' ~/.config/herdr/config.toml` → 2
- [x] T017 [REQ-009] Create `README.md` with sections: Install (git clone + `herdr plugin link` on each machine), Keybindings (cmd+shift+h / cmd+shift+s table), Review targets (semantics per REQ-002), Sending notes (behavior + agent resolution order). Verify: `grep -q 'herdr plugin link' README.md && grep -q 'cmd+shift+h' README.md && grep -q 'cmd+shift+s' README.md && grep -qi 'review target' README.md && grep -qi 'agent resolution' README.md` → exit 0
...
- [ ] H007 [REQ-002] Sub-pickers work: Pick commit, Pick range (`old>` then `new>`; `(worktree)` row → commit vs worktree), Branch vs branch two rounds
