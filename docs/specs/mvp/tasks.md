...
- [x] T008 [REQ-002] Wire interactive fzf flows (DEC-006): main menu fzf (first row default); Pick commit `git log --oneline --color=always -200 | fzf --ansi`; Pick range two rounds over the same log (`old>` picks the range base, `new>` lists only commits newer than it plus a leading `(worktree)` default row → `old..new`, worktree row → that commit vs worktree); Branch vs branch two-round fzf (`base>` then `compare>`) over `git branch -a --format='%(refname:short)' --sort=-committerdate` (`-a` required for remote-tracking branches; filter out the `origin/HEAD` alias row); Esc at any stage → exit 0. Verify: `python3 -m py_compile scripts/hunk_review.py` → exit 0 (interactive UX verified in Human Acceptance) *(amended 2026-08-18: originally shipped as single-round `--multi 2`)*
